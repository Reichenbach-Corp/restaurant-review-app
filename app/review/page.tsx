...
function ReviewForm() {
  ...
  const canSubmit = useMemo(() => Boolean(food && speed && service), [food, speed, service]);
  ...
  async function submit() {
    ...
    setResult(
      response.ok
        ? `Review accepted. ${data.points.total} points earned.`
        ...
        return (
          <section className="card" key={label}>
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                key={n}
                onClick={() => setter(n as Score)}
                aria-pressed={current === n}
                style={{ marginRight: 8, fontWeight: current === n ? 700 : 400 }}
              >
                {n}
              </button>
            ))}
      ...
      <section className="card">
        ...
        {lines.map((value, index) => (
          <input
            ...
            value={value}
            onChange={(event) =>
              setLines((old) => old.map((line, i) => (i === index ? event.target.value : line)))
            }
...
export default function ReviewPage() {
  return (
    <Suspense fallback={<main><p>Loading review…</p></main>}>
      <ReviewForm />
    </Suspense>
  );
}
